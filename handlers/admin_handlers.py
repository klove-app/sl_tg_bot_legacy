...
class AdminHandler(BaseHandler):
    ...
    def generate_report(self, year: int, month: int) -> str:
        ...
        for m, km, runs in month_data:
            bar = progress_bar(km, max_km, 8)
            report += f"│ {m:02d}.{year}   {runs:4d}   {km:7.1f}км │\n"
        ...
        if top_runners:
            ...
            for i, runner in enumerate(top_runners, 1):
                ...
                bar = progress_bar(runner_km, total_km, 8)
                
                medal = medals[i-1] if i <= 3 else "  "
                report += f"│{medal}{i:2d}. {username[:8]:<8} {runner_km:5.1f}км {percent:3.0f}% │\n"
                report += f"│ {bar} {runner['runs_count']:2d}x │\n"
        ...
        if challenges:
            ...
            for challenge in challenges:
                ...
                bar = progress_bar(total_km, goal_km, 8)
                
                title = challenge.title[:10] if challenge.title else "Без имени"
                report += f"│ {title:<10} {goal_km:5.0f}км        │\n"
                report += f"│ {bar} {progress:3.0f}% │\n"
        ...
        if chat_stats:
            ...
            for chat in chat_stats:
                ...
                bar = progress_bar(chat_km, max_chat_km, 8)
                percent = (chat_km / max_chat_km * 100)
                
                report += f"│ {chat_id_short:<6} {chat['runs_count']:4d}  {chat_km:6.1f}км {chat['users_count']:2d} │\n"
                report += f"│ {bar} {percent:3.0f}% │\n"
