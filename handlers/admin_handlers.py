...
class AdminHandler(BaseHandler):
    ...
    def generate_report(self, year: int, month: int) -> str:
        ...
        for m, km, runs in month_data:
            bar = progress_bar(km, max_km, 4)
            report += f"│ {m:02d}.{year}  {runs:4d} {km:6.1f}км  │\n"
            if m < len(month_data):
                report += "├──────────────────────┤\n"
        report += "└──────────────────────┘</pre>\n\n"
        ...
        if top_runners:
            ...
            for i, runner in enumerate(top_runners, 1):
                ...
                report += f"│{medal}{i:2d}. {username[:7]:<8}{runner_km:5.1f}км  │\n"
        ...
        if challenges:
            ...
            for challenge in challenges:
                ...
                report += f"│ {title:<8} {goal_km:4.0f}км {progress:3.0f}%  │\n"
        ...
        if chat_stats:
            ...
            for chat in chat_stats:
                ...
                report += f"│ {chat_id_short:<6} {chat['runs_count']:4d} {chat_km:5.1f}км  │\n"
