...
class AdminHandler(BaseHandler):
    ...
    def generate_report(self, year: int, month: int) -> str:
        ...
        report += "┌────────────────────────┐\n"
        report += "│ Месяц  Проб.  Дист.  % │\n"
        report += "├────────────────────────┤\n"
        
        for m, km, runs in month_data:
            bar = progress_bar(km, max_km, 6)
        ...
        if top_runners:
            ...
            for i, runner in enumerate(top_runners, 1):
                ...
                bar = progress_bar(runner_km, total_km, 4)
                
                medal = medals[i-1] if i <= 3 else " "
                report += f"│{medal}{i:2d}. {username[:7]:<7} {runner_km:5.1f}км {bar} │\n"
        ...
        if challenges:
            ...
            for challenge in challenges:
                ...
                bar = progress_bar(total_km, goal_km, 4)
                
                title = challenge.title[:8] if challenge.title else "Без имени"
                report += f"│ {title:<8} {goal_km:4.0f}км {bar} {progress:3.0f}% │\n"
        ...
        if chat_stats:
            ...
            for chat in chat_stats:
                ...
                bar = progress_bar(chat_km, max_chat_km, 4)
                percent = (chat_km / max_chat_km * 100)
                
                report += f"│ {chat_id_short:<6} {chat['runs_count']:4d} {chat_km:5.1f}км {bar} │\n"
