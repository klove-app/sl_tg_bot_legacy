...
class AdminHandler(BaseHandler):
    ...
    def generate_report(self, year: int, month: int) -> str:
        ...
        report += (
            ...
            f"│ 📏 Дистанция:  {total_km:6.1f} км     │\n"
            f"│ 👥 Участников:   {total_stats['users_count']:6d}      │\n"
        )
        if total_stats['runs_count'] > 0:
            report += f"│ ⚡️ Средняя:     {avg_km:6.1f} км     │\n"
        report += "└──────────────────────────────┘</pre>\n\n"
        
        # 2. Статистика по месяцам с графиком
        ...
        report += "┌──────────────────────────────┐\n"
        report += "│ Месяц  Пробежек   Дистанция │\n"
        report += "├──────────────────────────────┤\n"
        
        for m, km, runs in month_data:
            bar = progress_bar(km, max_km, 10)
            report += f"│ {m:02d}.{year}   {runs:5d}    {km:7.1f} км │\n"
            report += f"│ {bar} {(km/max_km*100):3.0f}%           │\n"
            if m < len(month_data):
                report += "├──────────────────────────────┤\n"
        report += "└──────────────────────────────┘</pre>\n\n"
        
        # 3. Топ пользователей с процентами от общей дистанции
        top_runners = RunningLog.get_top_runners(year=year, limit=10)
        if top_runners:
            report += "<b>🏆 ТОП-10 БЕГУНОВ</b>\n<pre>"
            report += "┌────────────────────────────────────┐\n"
            report += "│ Место   Бегун   Дист.  % от общего │\n"
            report += "├────────────────────────────────────┤\n"
            ...
            for i, runner in enumerate(top_runners, 1):
                ...
                report += f"│ {i:2d}. {username[:8]:<8} {runner_km:6.1f}км {percent:6.1f}% │\n"
                report += f"│    {bar} {runner['runs_count']:3d} пробежек    │\n"
                if i < len(top_runners):
                    report += "├────────────────────────────────────┤\n"
            report += "└────────────────────────────────────┘</pre>\n\n"
        
        # 4. Активные челленджи с прогресс-барами
        ...
        if challenges:
            ...
            for challenge in challenges:
                ...
                bar = progress_bar(total_km, goal_km)
                
                title = challenge.title[:12] if challenge.title else "Без имени"
                report += f"│ {title:<12} {goal_km:5.0f}км             │\n"
                report += f"│ {bar} {progress:5.1f}% │\n"
                if challenge != challenges[-1]:
                    report += "├────────────────────────────────────┤\n"
            report += "└────────────────────────────────────┘</pre>\n\n"
        
        # 5. Статистика по чатам с визуализацией
        chat_stats = RunningLog.get_chat_stats_all(year)
        if chat_stats:
            report += "<b>💬 СТАТИСТИКА ПО ЧАТАМ</b>\n<pre>"
            report += "┌────────────────────────────────────┐\n"
            report += "│ Чат     Пробежки  Дист.   Участн. │\n"
            report += "├────────────────────────────────────┤\n"
            ...
            for chat in chat_stats:
                ...
                report += f"│ {chat_id_short:<6} {chat['runs_count']:5d}   {chat_km:6.1f}км  {chat['users_count']:3d}  │\n"
                report += f"│ {bar} {percent:5.1f}%        │\n"
